...
public class AtualizarEquipaController {
    @FXML
    ...
    public void initialize() {
        try {
            // Obter a equipa a atualizar
            equipa = pt.ul.fc.css.soccernow.javaFX.fxControllers.GestaoEquipasController.getEquipaParaAtualizar();

            // Preencher o campo do nome
            if (equipa != null) {
                nomeField.setText(equipa.getNome());
                // Preencher a lista de jogadores da equipa
                if (equipa.getJogadores() != null) {
                    jogadoresEquipa.setAll(equipa.getJogadores());
                }
            }

            // Carregar todos os jogadores disponíveis
            todosJogadores = utilizadorController.listarJogadores();
            // Jogadores disponíveis = todos - já na equipa
            List<String> nomesJogadores = todosJogadores.stream().map(JogadorDTO::getUsername).toList();
            jogadoresDisponiveis.setAll(nomesJogadores);
            jogadoresDisponiveis.removeAll(jogadoresEquipa);
    ...
    private void guardar() {
        ...
        try {
            ...
            equipa.setNome(nome);
            equipa.setJogadores(jogadoresEquipa.stream().collect(Collectors.toList()));
            if (pt.ul.fc.css.soccernow.javaFX.fxControllers.GestaoEquipasController.getEquipaParaAtualizar() == null) {
                controller.registarEquipa(equipa);
                mensagemLabel.setText("Equipa registada!");
            } else {
                controller.atualizarEquipa(equipa.getId(), equipa);
                mensagemLabel.setText("Equipa atualizada!");
            }
            AppFX.setRoot("gestaoEquipas");
        } catch (Exception ex) {
            mensagemLabel.setText("Erro ao guardar equipa.");
        }
    }
